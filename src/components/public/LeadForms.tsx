...
import { publishedList, waLink } from "@/lib/cms";
...
function Sent({
  ...
  const hasWhatsApp = Boolean(whatsapp && whatsappMessage);
      ...
      {hasWhatsApp ? (
        <div className="mt-6">
          <p className="mb-3 text-sm font-medium text-navy">
            You can now continue with our team on WhatsApp.
          </p>

          <Button asChild variant="gold" size="lg">
            <a
              href={waLink(whatsapp, whatsappMessage!)}
              target="_blank"
...
export function InquiryForm({
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      return toast.error(
        "We couldn't send your message. Please try WhatsApp instead.",
      );
    }
  ...
  if (done) {
    return (
      <Sent
        text="Your inquiry has been recorded. You can continue with our team on WhatsApp to discuss your request."
        onReset={() => {
          setDone(false);
          setWhatsappMessage("");
        }}
...
export function BookingForm({
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      return toast.error(
        "We couldn't book that slot. Please try WhatsApp instead.",
      );
    }
    ...
    toast.success(
      "Consultation requested — we'll confirm your slot shortly.",
    );
  ...
  if (done) {
    return (
      <Sent
        text="Your consultation request has been recorded. Continue to WhatsApp so our team can discuss and confirm your appointment."
        onReset={() => {
          setDone(false);
          setWhatsappMessage("");
        }}
...
export function ContactMessageForm() {
  ...
  if (done) {
    return (
      <Sent
        text="Your message has been recorded. You can now continue directly with our team on WhatsApp."
        onReset={() => {
          setDone(false);
          setWhatsappMessage("");
        }}
