...
function Overview() {
  ...
  const { data: activity } = useQuery({
    ...
    queryFn: async () => {
      ...
      return [
        ...(posts.data ?? []).map((p) => ({
          text: `Blog post updated — ${p.title}`,
          at: p.updated_at,
        })),
        ...(msgs.data ?? []).map((m) => ({
          text: `New contact message from ${m.full_name}`,
          at: m.created_at,
        })),
        ...(reviews.data ?? []).map((r) => ({
          text: `New review from ${r.customer_name}`,
          at: r.created_at,
        })),
      ]
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Everything on the public website is managed from here.
        </p>
      </div>
