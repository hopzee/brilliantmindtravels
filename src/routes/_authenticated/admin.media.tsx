...
function Page() {
  ...
  const onUpload = async (files: FileList | null) => {
    ...
    setBusy(true);
    try {
      for (const file of Array.from(files)) await uploadMedia(file, folder || "general");
      toast.success("Upload complete");
      void qc.invalidateQueries({ queryKey: ["media_library"] });
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Images and videos used across the website.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          />
          <Button variant="gold" disabled={busy} onClick={() => inputRef.current?.click()}>
      ...
      <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-5">
        {(data ?? []).map((m) => (
          <div key={m.id} className="overflow-hidden rounded-xl border border-border bg-card">
            {m.mime_type?.startsWith("video") ? (
              ...
              <img src={m.url} alt={m.alt_text ?? m.title ?? ""} loading="lazy" className="h-32 w-full object-cover" />
            )}
        ...
        {(data?.length ?? 0) === 0 && <p className="text-sm text-muted-foreground">No media uploaded yet.</p>}
      </div>
    </div>
  );
}