...
import { EmptyState, PageHero, Reveal, SectionHeading } from "@/components/public/ui";
...
const title = "Client Reviews | Brilliant Mind Travels & Tours";
const description = "Read verified client reviews and share your own experience with our consultancy.";
...
const schema = z.object({
  ...
  content: z.string().trim().min(10, "Tell us a little more").max(1500),
});
...
function ReviewsPage() {
  ...
  return (
    <SiteLayout>
      <PageHero eyebrow="Reviews" title="What our clients say" intro="Verified reviews from people we've served." />
      <section className="bg-background py-20">
        <div className="container-page grid gap-14 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <SectionHeading eyebrow="Recent" title="Client reviews" />
            <div className="mt-8 space-y-5">
              {!data?.length ? (
                <EmptyState title="No reviews yet" text="Be the first to share your experience with us." />
              ) : (
                data.map((r: any, i: number) => (
                  <Reveal key={r.id} delay={i * 50}>
                    <article className="rounded-xl border border-border bg-card p-6">
                      ...
                      <p className="mt-4 text-sm font-semibold text-navy">{r.customer_name}</p>
                    </article>
                  </Reveal>
                ))
              )}
            ...
            {done ? (
              ...
              <form onSubmit={submit} className="mt-5 space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="content" name="content" rows={5} required maxLength={1500} />
                </div>
