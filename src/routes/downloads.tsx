...
import { CardSkeletons, EmptyState, PageHero, Reveal, SectionHeading } from "@/components/public/ui";
...
const title = "Download Centre | Brilliant Mind Travels & Tours";
const description =
  "Free checklists and guides: visa document checklists, study abroad guides and travel preparation resources.";
...
export const Route = createFileRoute("/downloads")({
  head: () => ({
    meta: [
      ...
      { property: "og:url", content: "https://brilliantmindtravels.lovable.app/downloads" },
      { name: "twitter:card", content: "summary" },
    ],
    links: [{ rel: "canonical", href: "https://brilliantmindtravels.lovable.app/downloads" }],
  }),
...
function DownloadCard({ item }: { item: any }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-card transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-elegant)]">
      {item.cover_image ? (
        <SmartImage src={item.cover_image} alt={item.title} ratio="aspect-16/10" fit="contain" />
      ) : null}
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-4 text-xs uppercase tracking-[0.14em] text-muted-foreground">
          {[item.file_type?.toUpperCase(), size, uploaded].filter(Boolean).join(" · ")}
        </p>
        <Button asChild variant="gold" size="lg" className="mt-5 w-full">
          <a href={item.file_url} target="_blank" rel="noopener noreferrer" download onClick={record}>
...
function DownloadsPage() {
  ...
  const items = data ?? [];
  const categories = Array.from(new Set(items.map((d: any) => d.category || "General")));
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        intro="Checklists and guides prepared by our consultants. Free to download, no sign up required."
      />
        ...
        <div className="container-page space-y-16">
          {isLoading ? (
            ...
            <EmptyState
              ...
              text="Our consultants are preparing checklists and guides for download."
            />
          ) : (
            categories.map((category, index) => (
              <div key={category}>
                ...
                <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {items
                    .filter((d: any) => (d.category || "General") === category)
                    .map((d: any, i: number) => (
                      <Reveal key={d.id} delay={(index + i) * 50}>
                        <DownloadCard item={d} />
                      </Reveal>
                    ))}
                </div>
